...
public static class EnumUtils
{
    ...
    internal static void Initialize(Harmony harmony, ManualLogSource logger)
    {
        try
        {
            harmony.Patch(AccessTools.Method(Type.GetType("System.Enum"), "GetCachedValuesAndNames"), null, null, new HarmonyMethod(AccessTools.Method(typeof(EnumInfoPatch), nameof(EnumInfoPatch.Transpiler))));
        }
        catch (Exception ex)
        {
            logger.LogError($"Exception while patching System.Enum.GetCachedValuesAndNames: {ex}");
        }
    }
